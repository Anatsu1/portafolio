...
| **Manarem** | HTML, CSS, JavaScript, Python, Flask, PostgreSQL, SQLite, gunicorn, API de AniList (consumo para anime) | `HTML5, CSS3, JavaScript, Python, Flask, PostgreSQL, SQLite`. gunicorn y el consumo de la API van en el resumen. **Acá PostgreSQL sí va** (a diferencia del VPS): el esquema, las consultas y la elección de motor son parte del proyecto, no un servicio levantado |
